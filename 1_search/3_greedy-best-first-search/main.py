...
class Frontier:
    ...
    def remove(self, goal) -> Node:
        return self.frontier.pop(self.get_closest_to(goal))
    ...
    def get_closest_to(self, goal) -> Node:
        closest = 0
        min = self.manhattan_distance(self.frontier[0].state, goal)
        for i in range(1, len(self.frontier)):
            d = self.manhattan_distance(self.frontier[i].state, goal)
            if min > d:
                min = d
                closest = i
        return closest
...
if __name__ == "__main__":
    ...
    prj_path = "./1_search/3_greedy-best-first-search/"
