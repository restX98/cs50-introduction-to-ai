...
class Frontier:
    ...
    def remove(self, goal) -> Node:
        return self.frontier.pop(self.estimate_best_node(goal))
    ...
    def estimate_best_node(self, goal) -> Node:
        closest = 0
        min = (
            self.manhattan_distance(self.frontier[0].state, goal)
            + self.frontier[0].cost
        )
        for i in range(1, len(self.frontier)):
            estimation = (
                self.manhattan_distance(self.frontier[i].state, goal)
                + self.frontier[i].cost
            )
            if min > estimation:
                min = estimation
                closest = i
...
class MazeSolver:
    ...
    def solve(self):
        ...
        while True:
            ...
            node = self.frontier.remove(self.maze.goal)
...
if __name__ == "__main__":
    filenames = ["maze", "maze1", "maze2", "maze3", "maze4"]
    prj_path = "./1_search/4_a_star_search/"
